...
L.Projection.wwsGlobal = L.Class.extend({
  ...
  project: function (latlng) {
    return new L.Point( (latlng.lng+180)*this.scaleparam, (90-latlng.lat)*this.scaleparam );
  },

  unproject: function (point) {
    return new L.LatLng( 90 - point.y/this.scaleparam, point.x/this.scaleparam - 180 );
  }
});
...
L.Map.ContextMenu = L.Handler.extend({
    ...
    _showAtPoint: function (pt, data) {
        if (!this._visible && this._items.length) {
            ...
            this._showLocation = { latlng: latlng, layerPoint: layerPoint, containerPoint: pt };
...
L.MapUtil = {
  ...
  fullView: function( map ){
    var center = new L.LatLng(39.0898, 107.22656, true);
    map.setView(center, 4);
  },

  refresh: function( map ){
    var layer = map._baselayer;
    layer.redraw();
  },
  ...
  showAttPopup: function( map, pos, data ){
    var tolerance = ( map.options.bound_maxx - map.options.bound_minx ) / 512.0;
    if( data.geo ){
      var latlngs = new Array();
      var vals = data.geo.split(',');
      for( var i=0; i<vals.length-1; i=i+2 ){
          var latlng = new L.LatLng(vals[i], vals[i+1]);
          latlng.lat -= tolerance;
          latlng.lng += tolerance;
          latlngs.push( latlng );
      }
    ...
    if( data.att ){
      var attHtml = '<h3>属性</h3><p>';
      var vals = data.att.split(',');
      for( var i=0; i<vals.length-1; i=i+2 ){
        attHtml += '[ ' + vals[i];
        attHtml += ' ]：' + '<strong>' + vals[i+1] + '</strong>';
        attHtml += '</br>';
      }
      attHtml += '</br></p>';
      var popup = L.popup().setLatLng(new L.LatLng(pos.lat-tolerance,pos.lng+tolerance)).setContent(attHtml).openOn(map);
    }
  },
  
  event_getCoordinates: function(e){//add by yq,返回经纬度
	  return e.latlng.lat + ',' + e.latlng.lng; 
  },

  event_showCoordinates: function(e){
      alert('X:' + e.latlng.lat + ' , Y:' + e.latlng.lng);
  },

  event_centerMap: function(e){
      map.panTo(e.latlng);
  },

  event_zoomIn: function(e){
      map.zoomIn();
  },

  event_zoomOut: function(e){
      map.zoomOut();
  },

  event_fullView: function(e){
      L.MapUtil.fullView(map);
  },

  event_refresh: function(e){
      L.MapUtil.refresh(map);
  },

  event_clearMap: function(e){
      L.MapUtil.clearMap(map);
  }
...
L.Control.Measure = L.Control.extend({
    ...
    getDistance: function( co1, co2 ){
        return Math.sqrt( (co1.lat-co2.lat)*(co1.lat-co2.lat) + (co1.lng-co2.lng)*(co1.lng-co2.lng) )
    },

    repairToler: function( latlng ){
        return latlng;
        //var toler = ( map.options.bound_maxx - map.options.bound_minx ) / 512.0;
        //return new L.latLng( latlng.lat-toler, latlng.lng+toler);
    },
    ...
    onAdd: function (map) {
        ...
        this._createButton('&#8674;', '测距', 'leaflet-control-measure leaflet-bar-part leaflet-bar-part-top-and-bottom', container, this._toggleMeasure, this);
    ...
    _mouseMove: function(e) {
        ...
        var latlng = this.repairToler( e.latlng );

        if(!this._layerPaintPathTemp) {
            this._layerPaintPathTemp = L.polygon([this._lastPoint, latlng], { 
                color: 'yellow',
                weight: 3,
                clickable: true,
                dashArray: '6,3'
            ...
            this._layerPaintPathTemp.spliceLatLngs(0, 2, this._lastPoint, latlng);
        }
    },
    ...
    _finishPath: function() {
        ...
        //alert( JSON.stringify(this._layerPaintPath.toGeoJSON()) );
...
/*
 * extension: DefaultEventHandler
 * L.control.defaultEventHandler().addTo(map);
 * 在地图做下角显示当前鼠标在地图上的经纬度
 */
L.Control.DefaultEventHandler = L.Control.extend({
  options: {
    act_position: true,
    act_select: true,
    act_key: true,
  ...
  _onMouseMove: function (e) {
    ...
    var prefixAndValue = this.options.prefix + ' ' + value + ' (' + zoom + ')';
    this._container.innerHTML = prefixAndValue;
  },
  
  _onKeyDown: function (e) {
    if (e.keyCode === 27) {
      L.MapUtil.clearMap(this._map);
    }
  }
...
L.Map.addInitHook(function () {
    if (this.options.defaultEventHandler) {
        this.defaultEventHandler = new L.Control.DefaultEventHandler();
        this.addControl(this.defaultEventHandler);
    }
});

L.control.defaultEventHandler = function (options) {
    return new L.Control.DefaultEventHandler(options);
};
