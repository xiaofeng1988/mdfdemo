...
function WebOffice1_NotifyToolBarClick(iIndex){
	...
	if(32772==iIndex) { //本地保存 这里改为保存到服务器
...
function saveToServer(){
	...
	var path = '${path}';
	var fillName = path.substr(path.lastIndexOf("/")+1,path.length);
	
	var ss = document.all.WebOffice1.SaveAs("C:\\test.doc", 0);
	alert(ss);
	...
	// 添加相应的Post元素 
	document.all.WebOffice1.HttpAddPostString("categoryId", "74");
	// 提交上传文件
	//var result = document.all.FramerControl1.HttpPost("<%=root%><%=path%>/productionDispatch/disFillTemplate/updateTemplate");
	//returnValue = document.all.WebOffice1.HttpPost("http://localhost:8080/zmsys/docs/uploadEditFile");
	returnValue = document.all.WebOffice1.HttpPost("http://localhost:8080/zmsys/productionDispatch/disFillTemplate/updateTemplate");
