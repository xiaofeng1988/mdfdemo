...
<html>
	<head>
		...
		<script type="text/javascript">
			$(function(){
				// 周恪竭：根据nodeid获取子节点信息
				$("#alltreeId_Module").tree({
					url:'${rootPath }/apiSysModuleCtr/tree/findsysModuleTrees',
					// 当中当前节点的 时候显示当前节点的详细信息
					onClick:function(node){
						//alltreeId
						$("#alltreeId").tree({
					        url:'${rootPath }/reqUrlCtr/apiReqUrl/'+node.id+'/tree',
					        onClick:function(node){
					        	$.post('${rootPath}/reqUrlCtr/apiReqUrl/'+node.id+'/detail', null, function(data){
								$("#detailId").html(data);
...
// 								console.log(data);
				...
				$("#addId").click(function(){
					var node = $('#alltreeId_Module').tree('getSelected');
					var nodeid = 0;
					// 判断当前树中是否有选中节点，如果有就是在下面建立子节点，如果没有就是建立一级菜单，默认是添加一级菜单
					if(node){
						nodeid = node.id; // 如果当前节点为真就在当前节点下添加节点
					}
					$("#adddeptpanel901").dialog("refresh", '${rootPath}/reqUrlCtr/add/'+nodeid+'/load');
				...
				$("#deleteId").click(function(){
					...
					if(node){
						$.messager.confirm("警告", "你确定要删除当前数据吗?" , function(r){
							if(r){
								$.post("${rootPath}/reqUrlCtr/deleteApiReqUrl/"+node.id, null, function(data){
	...
	<body>
        <div class="easyui-layout" data-options="fit:true,border:false" >
            ...
            <div data-options="border:true,region:'west',split:true" title="模块区域" style="width: 200px;">
                <ul id="alltreeId_Module"></ul>
            </div>
            <div data-options="border:true,region:'east',split:true" title="API接口" style="width: 200px;">
                <ul id="alltreeId"></ul>
            </div>
