			 ...
			 var iCount=getAutoRowCount();  
		 	 $("#tabId").datagrid({ 
				...
				//	pageList:[iCount,2*iCount,3*iCount],
				    ...
				    url:'${rootPath}/apiSysCtr/findAllApiSys',  
				    columns:[[   
				        {field:'ck',checkbox:true},  
				        {field:'id',hidden:true},     
				        {field:'sName',title:'系统',width:$(this).width() * 0.2}, 
				        {field:'sDes',title:'描述',width:$(this).width() * 0.3}
				    ]],
		 	 ...
		 	 //初始化弹出dialog窗口 
			    ...
			    function saveDialog(){ 
				 //非空校验 
				   	$("#addFormId").form('submit',{
				   		url:'${rootPath }/apiSysCtr/saveApiSys',
				   		onSubmit:function (){
				...
				$("#addId").click(function(){
					// 加载数据
					$('#crudDivId').dialog('refresh', '${rootPath}/apiSysCtr/add/load');  
					// 设置title
					$("#crudDivId").dialog({
						title:'添加数据',
				...
				$("#editId").click(function(){
				     var rows = $("#tabId").datagrid('getChecked');  
		       		 if(rows.length==1){	
							...
							// 加载数据
							$('#crudDivId').dialog('refresh', '${rootPath}/apiSysCtr/edit/load?id='+row.id);  
							// 设置title
							$("#crudDivId").dialog({
								title:'修改数据',
		        	 ...
		        	 if(rows.length>0){
		        		$.messager.confirm('提示', '确定要删除数据?',function(r){
		        			if(r){
		        				 ...
		        				 $.post('${rootPath}/apiSysCtr/deleteApiSys',{ids:idss},function(data){
		        					 var result=eval('('+data+')');
		        					 if (result.success){
		        						 $("#tabId").datagrid('reload');
		        					 }else{
